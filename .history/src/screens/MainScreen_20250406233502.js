...
const MapScreen = () => {
  ...
  const shareLocation = async () => {
    try {
      ...
      if (!profileData) {
        Alert.alert('Error', 'Please set up emergency contacts in your profile first');
        return;
      }
      ...
      if (isAvailable) {
        const validContacts = emergencyContacts.filter(contact => contact.phone);
        const phones = validContacts.map(contact => contact.phone);
      ...
      <View style={styles.emergencyButtonContainer}>
        <TouchableOpacity 
          style={[
            ...
            isSharingLocation && styles.emergencyButtonDisabled
          ]}
        ...
        >
          <View style={styles.emergencyButtonInner}>
            <FontAwesome5 
              name={isSharingLocation ? "spinner" : "exclamation-triangle"} 
              size={32} 
              color="#FFF" 
            />
            <Text style={styles.emergencyButtonText}>
              {isSharingLocation ? 'SENDING ALERT...' : 'EMERGENCY'}
            </Text>
            <Text style={styles.emergencyButtonSubtext}>
              {isSharingLocation ? 'Notifying contacts' : 'Press for immediate help'}
            </Text>
          </View>
        </TouchableOpacity>
      </View>
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: 'Remember to share your location with trusted contacts when traveling alone.',
      time: '2 hours ago',
      type: 'reminder'
    },
    {
    ...
    },
    {
      id: '3',
      title: 'Safety Tip',
      message: 'Keep your phone charged and emergency contacts updated.',
      time: '2 days ago',
      type: 'tip'
    }
  ]);
