  ...
  const loadEmergencyContacts = async () => {
    try {
      ...
      if (profileData) {
        ...
        const validContacts = parsedData.emergencyContacts?.filter(contact => contact.phone || contact.phoneNumber) || [];
        const formattedContacts = validContacts.map(contact => ({
          id: contact.id,
          name: contact.name,
          phone: contact.phone || contact.phoneNumber // Handle both phone and phoneNumber fields
        }));
