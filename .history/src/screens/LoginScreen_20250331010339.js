...
const LoginScreen = ({ navigation }) => {
  ...
  const [request, response, promptAsync] = Google.useIdTokenAuthRequest({
    clientId: '686962153219-8ikf8ml6cd6qtq9nq2lmvjkv22seqkp1.apps.googleusercontent.com',
  ...
  const handleDevSignIn = async () => {
    try {
      ...
      await AsyncStorage.setItem('userToken', 'dev-user-token');
      navigation.replace("MainApp");
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        ...
        {__DEV__ && (
          <TouchableOpacity 
            style={[styles.iconButton, { marginBottom: 10 }]} 
        ...
        <TouchableOpacity 
          ...
          onPress={() => Alert.alert('Note', 'Google Sign-In is only available in production build')}
          disabled={loading}
        >
