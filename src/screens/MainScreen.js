...
const MapScreen = () => {
  ...
  const loadEmergencyContacts = async () => {
    try {
      ...
      if (profileData) {
        ...
        // Use the same contacts that are shown in the Contacts screen
        const validContacts = (parsedData.emergencyContacts || []).filter(contact => 
          contact && contact.name && contact.phone
        );
        setEmergencyContacts(validContacts);

        // If no contacts are loaded, show an alert
        if (validContacts.length === 0) {
          Alert.alert(
            'No Emergency Contacts',
            'Please add emergency contacts in the Contacts tab first.',
            [
              { 
                text: 'OK',
  ...
  const handleSharePress = async () => {
    // First load/refresh the emergency contacts
    await loadEmergencyContacts();
    
    // Only show the modal if there are contacts
...
const AlertsScreen = () => {
  ...
  useEffect(() => {
    ...
    // Set up notification handler for when app is running
    const foregroundSubscription = Notifications.addNotificationReceivedListener(handleNewNotification);
    
    // Set up notification handler for when app is in background
    const backgroundSubscription = Notifications.addNotificationResponseReceivedListener(handleNewNotification);
  ...
  const handleNewNotification = async (notification) => {
    try {
      ...
      const newNotification = {
        ...
        title: notificationData.request.content.title,
        body: notificationData.request.content.body,
        type: notificationData.request.content.data?.type || 'alert',
...
const ContactsScreen = () => {
  ...
  const loadEmergencyContacts = async () => {
    try {
      const profileData = await AsyncStorage.getItem('userProfile');
      if (profileData) {
        const { emergencyContacts } = JSON.parse(profileData);
        // Filter out any null or undefined contacts
        const validContacts = (emergencyContacts || []).filter(contact => 
          contact && contact.name && (contact.phone || contact.phoneNumber)
        );
        setEmergencyContacts(validContacts);
      }
    } catch (error) {
  ...
  const saveEmergencyContacts = async (contacts) => {
    try {
      ...
      // Update the profile with new emergency contacts
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.deviceContactsList}>
              {filteredContacts.map((contact) => (
                ...
                >
                  <View style={styles.deviceContactIconContainer}>
                    <FontAwesome5 name="user" size={24} color="#3D212B" />
                  </View>
...
const ProfileScreen = () => {
  ...
  const saveProfile = async () => {
    try {
      ...
      // Format emergency contacts to ensure consistent structure
